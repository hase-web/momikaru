...
export const handler = async (event) => {
  ...
  try {
    ...
    const brandLabel = process.env.BRAND_LABEL || "【もみかる採用】";

    const description = [
      `${brandLabel}${eventConfig.label}`,
      `お名前: ${name}`,
      `メール: ${email}`,
      phone ? `電話: ${phone}` : null,
      jobCategory ? `応募職種: ${jobCategory}` : null,
      experience ? `経験年数: ${experience}` : null,
      area ? `希望エリア: ${area}` : null,
      timing ? `希望時期: ${timing}` : null,
    ...
    const result = await createBookingEvent(calendar, staff.calendarId, {
      ...
      summary: `${brandLabel}${eventConfig.label} — ${name} 様`,
      description,
      attendeeEmail: email,
      attendeeName: name,
    });
