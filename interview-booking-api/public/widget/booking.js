(function () {
  ...
  function open(options) {
    options = options || {};
    state.eventType = options.eventType || "interview";
    state.staffId = options.staffId || "any";
    state.jobCategory = options.jobCategory || "";
  ...
  function renderFormFields(ev) {
    if (cfg.formMode === "recruit") {
      var jobVal = state.jobCategory || "";
      return (
        ...
        '<div class="fc-booking-field"><label>電話番号 <span style="color:#c41e24">*</span></label><input name="phone" type="tel" required autocomplete="tel"></div>' +
        '<div class="fc-booking-field"><label>応募職種</label><input name="jobCategory" value="' +
        jobVal.replace(/"/g, "&quot;") +
        '" placeholder="例：整骨・整体"></div>' +
        '<div class="fc-booking-field"><label>経験年数</label><select name="experience"><option value="">選択</option><option>未経験</option><option>1〜3年</option><option>3〜5年</option><option>5年以上</option></select></div>' +
        '<div class="fc-booking-field"><label>備考・質問</label><textarea name="message" placeholder="ご質問や希望があればご記入ください"></textarea></div>'
      );
    }
    return (
      '<div class="fc-booking-field"><label>お名前 <span style="color:#c41e24">*</span></label><input name="name" required autocomplete="name"></div>' +
  ...
  function submitBooking() {
    ...
    if (cfg.formMode === "recruit" && !get("phone")) {
      alert("電話番号は必須です");
      return;
    }
    if (!state.selectedSlot) {
    ...
    fetch(API + "/.netlify/functions/book", {
      ...
      body: JSON.stringify({
        ...
        area: get("area"),
        timing: get("timing"),
        jobCategory: get("jobCategory") || state.jobCategory,
  ...
  function detectEventType(el) {
    ...
    if (t.indexOf("オンライン") >= 0) return "interview_online";
    return cfg.formMode === "recruit" ? "interview" : "consult";
  }

  function bindTriggers() {
    document.querySelectorAll(".fc-booking-open, .iv-booking-open").forEach(function (el) {
      if (el.dataset.fcBound) return;
      el.dataset.fcBound = "1";
      el.addEventListener("click", function (e) {
        e.preventDefault();
        open({
          eventType: detectEventType(el),
          jobCategory: el.dataset.ivJob || "",
        });
      });
    });
  }

  window.FCBooking = window.IVBooking = { open: open, close: close };
